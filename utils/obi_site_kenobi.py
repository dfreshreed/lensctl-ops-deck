...
def lookup_or_create_site(csv_site_name: str):
    ...
    if data.get("errors"):
        logger.error(f"Error creating site: {data['errors']}")
    return data["data"]["upsertSite"]["id"]

def resolve_site(csv_site_id, csv_site_name, site_name_to_id: dict, site_id_to_name: dict):
    """
    ...
    if csv_site_id:
        ...
        current_name = site_id_to_name.get(csv_site_id) or fetch_site_name(csv_site_id)
